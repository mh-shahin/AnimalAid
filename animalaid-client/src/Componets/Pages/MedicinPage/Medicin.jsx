...
const Medicin = () => {
    ...
    useEffect(() => {
        ...
        if (priceRange.min !== '' || priceRange.max !== '') {
            ...
            filtered = Object.keys(filtered).reduce((obj, cat) => {
                const filteredProducts = filtered[cat].filter(med => {
    ...
    }, [selectedCategories, priceRange, groupedMedicines]);
    ...
    const totalProducts = Object.values(filteredMedicines).reduce((sum, products) => sum + products.length, 0);
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-green-600 text-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    <div className="text-center">
                        <h1 className="text-4xl sm:text-5xl font-bold mb-3">Medicines</h1>
                                    ...
                                    {Object.keys(groupedMedicines).map(category => (
                                        <label key={category} className="flex items-center gap-3 cursor-pointer group p-2 hover:bg-gray-50 rounded-lg transition">
                                            ...
                                            <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-full">
                                                {groupedMedicines[category].length}
                                            </span>
                                        </label>
                                    ))}
                                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {Object.keys(filteredMedicines).length === 0 ? (
                    ...
                    Object.keys(filteredMedicines).map((category) => (
                        <div key={category} className="mb-12">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-2xl font-bold text-gray-900">{category}</h2>
                                    <p className="text-sm text-gray-500 mt-1">
                                        {filteredMedicines[category].length} {filteredMedicines[category].length === 1 ? 'product' : 'products'} available
                                    </p>
                                </div>
                                {groupedMedicines[category] && groupedMedicines[category].length > 5 && (
                                    <Link
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                                {filteredMedicines[category].slice(0, 5).map((product) => (
